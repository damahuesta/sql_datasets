...
9. [James Bond](#9-james-bond)
10. [MovieLens](#10-movielens)
11. [RENFE](#11-renfe)
12. [Rolling Stone Top Songs](#12-rolling-stone-top-songs)
13. [Spotify Tracks](#13-spotify-tracks)
14. [Star Wars](#14-star-wars)
15. [Videogames](#15-videogames)
...
**Descripción**: Películas de James Bond con información de producción y taquilla.

**Tabla**: `james_bond`

| Campo | Tipo | Descripción |
|-------|------|-------------|
| Film | STRING | Título de la película |
| Year | INTEGER | Año de estreno |
| Actor | STRING | Actor que interpreta a Bond |
| Director | STRING | Director |
| Box_Office | STRING | Recaudación mundial (millones USD) |
| Budget | STRING | Presupuesto (millones USD) |
| Bond_Actor_Salary | STRING | Salario del actor (millones USD) |

**Nota**: Los campos numéricos están en formato STRING con comas como separador decimal.

---

## 10. MovieLens

**Descripción**: Dataset de recomendación de películas con valoraciones y etiquetas de usuarios.
...
| Campo | Tipo | Descripción |
|-------|------|-------------|
| movieId | INTEGER | ID único de la película |
| title | STRING | Título (incluye año) |
| genres | STRING | Géneros separados por \| |
...
| movieId | INTEGER | FK a movies |
| rating | FLOAT | Valoración (0.5-5.0) |
...
**Descripción**: Catálogo de estaciones de RENFE en España con ubicación geográfica.

**Tabla**: `renfe_estaciones`

| Campo | Tipo | Descripción |
|-------|------|-------------|
| Id | INTEGER | ID único de la estación |
| Codigo | INTEGER | Código RENFE |
| Descripcion | STRING | Nombre de la estación |
| Provincia | STRING | Provincia |
| Poblacion | STRING | Población |
| CodigoPostal | FLOAT | Código postal |
| Direccion | STRING | Dirección completa |
| Latitud | STRING | Latitud (coordenada) |
| Longitud | STRING | Longitud (coordenada) |
| Pais | STRING | País (España) |

---

## 12. Rolling Stone Top Songs

**Descripción**: Las mejores 500 canciones según Rolling Stone con métricas de Spotify.

**Tabla**: `rolling_top_songs`

| Campo | Tipo | Descripción |
|-------|------|-------------|
| Song_Index | INTEGER | Posición en el ranking |
| Song | STRING | Título de la canción |
| Artist | STRING | Artista |
| Track_No_ | INTEGER | Número de pista |
| AlbumID_Rank | INTEGER | Ranking del álbum |
| BPM | INTEGER | Pulsaciones por minuto |
| Energy | INTEGER | Nivel de energía |
| Dance | INTEGER | Capacidad de baile |
| Loud | INTEGER | Volumen/intensidad |
| Valence | INTEGER | Positividad musical |
| Duration | INTEGER | Duración (segundos) |
| Acoustic | INTEGER | Nivel acústico |
| Popularity | INTEGER | Popularidad en Spotify |
| Spotify_Song | STRING | URL de Spotify |

**Nota**: Algunas canciones tienen valores 0 en métricas de Spotify por no estar disponibles.
...
**Descripción**: Dataset de ventas de videojuegos por región y plataforma.

**Tabla**: `videogames_games`

| Campo | Tipo | Descripción |
|-------|------|-------------|
| rank | INTEGER | Posición en el ranking |
| name | STRING | Nombre del videojuego |
| platform_code | STRING | Código de plataforma (DS, PS4, Xbox, etc.) |
| year | INTEGER | Año de lanzamiento |
| genre | STRING | Género del juego |
| publisher | STRING | Empresa publicadora |
| NA_sales | STRING | Ventas en Norteamérica (millones) |
| EU_sales | STRING | Ventas en Europa (millones) |
| JP_sales | STRING | Ventas en Japón (millones) |
| Other_sales | STRING | Ventas en otras regiones (millones) |

**Nota**: Las ventas usan coma como separador decimal.
